<template>
	<view>
		...
		<view class="header-wrap">
			<view class="top">
				<view>
					<view class="name-style">姓名: </view>
					<view class="name-style-text">{{children_name}}</view>
				</view>
				<view>
					<view class="name-style">生日: </view>
					<view class="name-style-text">{{children_date}}</view>
				</view>
				<view>
					<view class="name-style">
						设备macDE:
					</view>
					<view class="name-style-text">{{deviceId}}</view>
				</view>
			</view>
			<view class="bottom">
				<u-button type="warning" @click="relieveData">解除绑定</u-button>
				<!-- <view class="first" @click="relieveData()">解除绑定</view> -->
				<!-- <view class="last" @click="notifyMy()">同步数据</view> -->
				<view class="" style="width: 40upx;"></view>
				<u-button :ripple='true' type="primary" @click="notifyMy">同步数据</u-button>
		...
		<view class="chart">
			<view class="chart-wrap">
				<view class="chart-wrap-list"    :class="item.id==count?'actives':''" @click="cate(item.id)"
					v-for="(item,index) in listYong" :key="index"> {{item.name}}</view>
		...
		<!-- 	<view class="classlist flex">
				<view @click="cate(1)"
					:style="{'background':(classcate == 1 ? '#FFB58D':'#ffffff'),'color':(classcate != 1 ? '#000':'#ffffff')}">
					日数据</view>
				<view @click="cate(2)"
					:style="{'background':(classcate == 2 ? '#FFB58D':'#ffffff'),'color':(classcate != 2 ? '#000':'#ffffff')}">
					周数据</view>
				<view @click="cate(3)"
					:style="{'background':(classcate == 3 ? '#FFB58D':'#ffffff'),'color':(classcate != 3 ? '#000':'#ffffff')}">
					月数据</view>
			</view> -->
			...
			<!-- 	<qiun-data-charts :style="visible?'display:none':''" type="column" :opts="opts" :chartData="chartData"
					:ontouch="true" /> -->
					<category-line  :count="count" :children_id="children_id"/>
			<!-- </view> -->
		<!-- 	<view class="charts-min" style="width: 98%;text-align: right;margin: 10rpx auto 100rpx auto;">时间/{{company}}
			</view> -->
...
<script>
	import CategoryLine from "@/components/categoryLine/index"
	var uChartsInstance = {};
	export default {
		components:{CategoryLine},
		data() {
			return {
...
<style lang="scss" scoped>
	...
	.chart-wrap {
		...
		.chart-wrap-list:nth-child(odd) {
			// margin-right: 10upx;
		}

		.chart-wrap-list:nth-child(even) {
			// margin-right: 10upx;
		}
